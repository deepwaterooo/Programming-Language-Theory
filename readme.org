...
* Introduction
- Todos: 
  - Don't like the 24 size vector, want to change to be a 8 something, so that it will be easier for me to manipulate other instance of objects as well. 
  - Try to find better way to handle obj% drawing methods, so that on-paint I may just call the obj's draw methods.
  - Sub obj% for cubes, and spheres if I need and want to implement any sphere for head, or eyes. 
  - Will add some others related later according to buddy & course instructor. 
  - Will update tonight or tomorrow to update status. 
- 
